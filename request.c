...
void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg, struct Statistics* stats) 
{
   ...
   // Create the body of the error message
	...
	sprintf(buf, "%sStat-Thread-Static:: %d\r\n", buf, stats->thread_static_count);

	sprintf(buf, "%sStat-Thread-Dynamic:: %d\r\n\r\n", buf, stats->thread_dynamic_count);
	Rio_writen(fd, buf, strlen(buf));
//	printf("%s", buf);
...
void requestServeDynamic(int fd, char *filename, char *cgiargs,  struct  Statistics* stats) //int filesize,
{
   stats->thread_dynamic_count++;
   char buf[MAXLINE], filetype[MAXLINE], *emptylist[] = {NULL};
...
void requestServeStatic(int fd, char *filename, int filesize, struct Statistics* stats) 
{
   stats->thread_static_count++;
   int srcfd;
...
void requestHandle(int fd, struct Statistics* stats)
{
   ...
   if (is_static) {
      ...
      requestServeDynamic(fd, filename, cgiargs, stats);// sbuf.st_size,
   }
}
